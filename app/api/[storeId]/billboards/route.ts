...
export const POST = async (req: Request, { params }: { params: { storeId: string } }) => {
    try {
        ...
        const billboardData = { label, imageUrl, createdAt: serverTimestamp() }

        const billboardRef = await addDoc(collection(db, 'stores', params.storeId, 'billboards'), billboardData);
...
export const GET = async (req: Request, { params }: { params: { storeId: string } }) => {
    try {
        ...
        const billboardsData = (await getDocs(collection(doc(db, 'stores', params.storeId), 'billboards'))).docs.map((doc) => doc.data() as Billboards[])
