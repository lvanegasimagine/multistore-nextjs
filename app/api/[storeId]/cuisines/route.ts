...
export const POST = async (req: Request, { params }: { params: { storeId: string } }) => {
    try {
        ...
        const cuisineData = { name, value, createdAt: serverTimestamp() }

        const cuisineRef = await addDoc(collection(db, 'stores', params.storeId, 'cuisines'), cuisineData);
...
export const GET = async (req: Request, { params }: { params: { storeId: string } }) => {
    try {
        ...
        const cuisineData = (await getDocs(collection(doc(db, 'stores', params.storeId), 'cuisines'))).docs.map((doc) => doc.data() as Cuisine[])
